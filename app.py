...
"""def read_sensor_data():
    ...
    try:
        ...
        print(f"Ã¢ÂÅ Error reading sensor: {e}")
        return None
"""
def read_sensor_data():
    """Read actual values from RS485 soil sensor or simulate if not connected."""
    if not instrument:
        # 🧪 Simulate random realistic values for testing (when RS485 not connected)
        return {
            "moisture": round(random.uniform(40, 80), 1),
            "temperature": round(random.uniform(20, 35), 1),
            "ph": round(random.uniform(5.5, 7.5), 1),
            "nitrogen": random.randint(60, 120),
            "phosphorus": random.randint(40, 90),
            "potassium": random.randint(80, 130),
            "timestamp": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        }
    
    # 🧩 Otherwise, use actual Modbus sensor
    try:
        moisture = instrument.read_register(0, 0) / 10.0
        temperature = instrument.read_register(1, 0) / 10.0
        ph = instrument.read_register(3, 0) / 10.0
        ...
        # Fallback to random test data
        return {
            "moisture": round(random.uniform(40, 50), 1),
            "temperature": round(random.uniform(28, 35), 1),
            "ph": round(random.uniform(6.5, 7.5), 1),
            "nitrogen": random.randint(80, 90),
            "phosphorus": random.randint(165, 170),
            "potassium": random.randint(170, 180),
            "timestamp": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        }
